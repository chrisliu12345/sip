...
public class Session
    ...
    private long LEASE_TIME_MILLIS = Config.getLongProperty("session.timeout.mins") * 60L * 1000L;
    private volatile long timeToLive = this.LEASE_TIME_MILLIS;

    public static String[] FORCED_PULL_AGENTS = Config.getProperty("listen.force.pull.agents").split(",");
    ...
    public static Session create(String anId)
        ...
        try {
            session = (Session) Config.getClass("session.class", "nl.justobjects.com.git.wuqf.pushlet.core.Session").newInstance();
        } catch (Throwable t) {
