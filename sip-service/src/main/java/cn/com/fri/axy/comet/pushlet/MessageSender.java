...
public class MessageSender {
    ...
    public synchronized void send(String paramString) {
        try {
            ...
            for (int i = 0; i < j; i++) {
                ...
                append(Byte.toString(paramString));
                append("|");
            }


            (paramString = Event.createDataEvent("/comet/sip")).setField("sip_message", toString());
            Dispatcher.getInstance().multicast(paramString);
            return;
        } catch (Exception localException) {
            SysLogger.info(this = localException);
        }
    }
    ...
    public synchronized void sendPlayNotify(String paramString1, String paramString2, int paramInt) {
        try {
            ...
            (
            MessageSender ms = Event.createDataEvent("/comet/play_notify").setField("play_notify_action", paramString1);
            setField("play_notify_ip", paramString2);
            setField("play_notify_port", paramInt);
            Dispatcher.getInstance().multicast(this);
            return;
        } catch (Exception localException) {
            SysLogger.info(this = localException);
        }
