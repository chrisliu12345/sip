...
import cn.com.fri.axy.common.def.message.batchmessage.SimpleObserver;
...
public class HttpAccessServlet
    ...
    public void service(HttpServletRequest paramHttpServletRequest, HttpServletResponse paramHttpServletResponse) {
        ...
        this = paramHttpServletResponse.getWriter();

        if (paramHttpServletRequest.getMethod().equals("POST")) {
            try {
                localObject = paramHttpServletRequest.getParameter("requestType");

                if ("sendmessage".equalsIgnoreCase((String) localObject)) {
                    localObject = new SimpleObserver();


                    paramHttpServletRequest.setCharacterEncoding("UTF-8");
                    paramHttpServletResponse = paramHttpServletRequest.getParameter("devicetype");
                    String str1 = paramHttpServletRequest.getParameter("deviceid");
                    String str2 = paramHttpServletRequest.getParameter("MessageParam");
                    paramHttpServletRequest = paramHttpServletRequest.getParameter("MessageTextAera");
